...
namespace lve {
	...
	VkCommandBuffer LveRenderer::beginFrame() {
		...
		//A surface has changed in such way it is no longer compatible with the current swap chain -> further presentation
		...
		if (result != VK_SUCCESS && result != VK_SUBOPTIMAL_KHR) {
			throw std::runtime_error("failed to acquire swap chin image");
		}
	...
	void LveRenderer::endSwapChainRenderPass(VkCommandBuffer commandBuffer) {
		...
		assert(commandBuffer == getCurrentCommandBuffer() && "Can't begin end pass on command buffer from a different frame");
